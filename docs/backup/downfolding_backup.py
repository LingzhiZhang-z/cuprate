...
def Spin_Flip_Plus(code):
    if code == 2:
        return 1
    return None


def Spin_Flip_Minus(code):
    if code == 1:
        return 2
    return None
...
def spin_matrix_J_ij(states, bond):
    ...
    for i, state in enumerate(states):
        code1 = site_code(state, site1)
        code2 = site_code(state, site2)
        SzSz[i, i] = Spin_z(code1) * Spin_z(code2)

        # S+S-
        fp1, fm2 = Spin_Flip_Plus(code1), Spin_Flip_Minus(code2)
        if fp1 is not None and fm2 is not None:
            flipped = set_site(set_site(state, site1, fp1), site2, fm2)
            j = state_index.get(flipped)
            if j is not None:
                SpSm[j, i] = 1.0

        # S-S+
        fm1, fp2 = Spin_Flip_Minus(code1), Spin_Flip_Plus(code2)
        if fm1 is not None and fp2 is not None:
            flipped = set_site(set_site(state, site1, fm1), site2, fp2)
            j = state_index.get(flipped)
            if j is not None:
                SmSp[j, i] = 1.0
...
def compute_t11_norm(eigvecs_selected, spin_basis):
    try:
        model_dim = spin_basis.shape[0]
        S_BD = spin_basis @ eigvecs_selected
        U, Sigma, VH = np.linalg.svd(S_BD, full_matrices=False)
        T11m1 = U @ np.diag(Sigma) @ U.conj().T - np.eye(model_dim)
        return np.linalg.norm(T11m1.flatten())
    except np.linalg.LinAlgError:
        return np.inf


def _block_t11_norm(eigvecs_blocks, selected_blocks, spin_basis_blocks):
    selected = np.concatenate(
        [ev[:, idx] for ev, idx in zip(eigvecs_blocks, selected_blocks)], axis=1
    )
    spin_basis = np.concatenate(spin_basis_blocks, axis=0)
    return compute_t11_norm(selected, spin_basis)
...
def select_by_occupation(double_occ_blocks, eigvecs_blocks, dimspin_blocks, spin_basis_blocks, eigvals_blocks=None):
    ...
    return selected, _block_t11_norm(eigvecs_blocks, selected, spin_basis_blocks)


def select_by_energy(double_occ_blocks, eigvecs_blocks, dimspin_blocks, spin_basis_blocks, eigvals_blocks):
...
def greedy_swap(init_blocks, others_blocks, eigvecs_blocks, spin_basis_blocks, log=None):
    selected = [idx.copy() for idx in init_blocks]
    swap = [oth.copy() for oth in others_blocks]

    best_norm = _block_t11_norm(eigvecs_blocks, selected, spin_basis_blocks)
    ...
    for blk in range(len(selected)):
        ...
        for i in range(len(selected[blk])):
            ...
            for j in range(len(swap[blk])):
                ...
                t0 = time.time()
                norm = _block_t11_norm(eigvecs_blocks, selected, spin_basis_blocks)
                if log is not None:
...
def select_greedy(double_occ_blocks, eigvecs_blocks, dimspin_blocks, ratio=5,
                  spin_basis_blocks=None, tmp_dir=None, twoSz=None, block_idx=None, eigvals_blocks=None):
    t0 = time.time()
    if eigvals_blocks is None:
        eigvals_blocks = [None for _ in double_occ_blocks]
    sorted_blocks = [_argsort_double_occ(do, ev) for do, ev in zip(double_occ_blocks, eigvals_blocks)]
    ...
    if tmp_dir is not None:
        ...
        with open(f"{log_stem}_single.txt", "w") as log:
            ...
            selected, _, best_norm = greedy_swap(init_blocks, others_blocks, eigvecs_blocks, spin_basis_blocks, log)
            log.write(f"Best |T11-1| = {best_norm:.12f}\n")
            log.write(f"Total time cost: {time.time() - t0:.1f}s\n")
    else:
        selected, _, best_norm = greedy_swap(init_blocks, others_blocks, eigvecs_blocks, spin_basis_blocks, None)
...
def select_multi_restart(double_occ, eigvecs, dimspin, N, U, t_value,
    ...
    best_norm = compute_t11_norm(eigvecs[:, best_selected], spin_basis)
            ...
            with _open_log_file(iter_path) as iter_log:
                ...
                if not flag_rand:
                    sel_blocks, swap_blocks, cur_norm = greedy_swap(
                        [best_selected], [best_swap], [eigvecs], [spin_basis], iter_log
                    )
                    ...
                    sel_blocks, swap_blocks, cur_norm = greedy_swap(
                        [cur_selected], [cur_swap], [eigvecs], [spin_basis], iter_log
                    )
                    cur_selected, cur_swap = sel_blocks[0], swap_blocks[0]
...
def select_adiabatic(eigvecs_blocks, dimspin_blocks, spin_basis_blocks, filename_eigvecs, filename_indices):
...
def select_eigenstates(method, global_indices, eigvals_blocks, eigvecs_blocks,
                       double_occ_blocks, dimspin_blocks, spin_basis_blocks,
    ...
    if method is None:
        selected_blocks, best_norm = select_by_occupation(
            double_occ_blocks, eigvecs_blocks, dimspin_blocks, spin_basis_blocks, eigvals_blocks
        )
        if np.isinf(best_norm):
        ...
        selected_blocks, best_norm = select_by_energy(
            double_occ_blocks, eigvecs_blocks, dimspin_blocks, spin_basis_blocks, eigvals_blocks
        )
        ...
        selected_blocks = []
        for block_idx, (double_occ_block, eigvecs_block, dimspin_block, spin_basis_block) in enumerate(
            zip(double_occ_blocks, eigvecs_blocks, dimspin_blocks, spin_basis_blocks)
        ):
        ...
        best_norm = _block_t11_norm(eigvecs_blocks, selected_blocks, spin_basis_blocks)
        ...
        selected_blocks, best_norm, overlap = select_adiabatic(
            eigvecs_blocks, dimspin_blocks, spin_basis_blocks, filename_eigvecs, filename_indices,
        )
    else:
...
def calc_double_occ_expectation(states, eigvecs, N):
    dom = calc_double_occupation_matrix(states, N)
    return np.diag(eigvecs.conj().T @ dom @ eigvecs)


def extract_heff(eigvals, eigvecs, selected_indices, spin_basis):
    """SVD-extract effective spin Hamiltonian.

    Returns (heff, t11m1, t11m1_norm).
    """
    model_dim = spin_basis.shape[0]
    S_BD = spin_basis @ eigvecs[:, selected_indices]
    U, Sigma, VH = np.linalg.svd(S_BD, full_matrices=False)
    t11m1 = U @ np.diag(Sigma) @ U.conj().T - np.eye(model_dim)
