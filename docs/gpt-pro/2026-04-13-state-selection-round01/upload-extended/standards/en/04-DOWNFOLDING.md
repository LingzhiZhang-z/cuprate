...
- From the full eigensystem, select $d_{\text{spin}}$ eigenstates whose projection onto the
  pure-spin subspace (rows $0$ to $d_{\text{spin}} - 1$) is as close to a unitary map as possible.
...
- The spin-projected matrix is $S_{BD} = P_{\text{spin}} \Psi_{\text{sel}}$
  (first $d_{\text{spin}}$ rows of $\Psi_{\text{sel}}$).
...
- All operators are constructed in the pure-spin subspace (`states[:dimspin]`).
